...
export async function getUser(uid){
  const response = await apiRequest(`user-get?uid=${uid}`);
  return response;
}
...
export async function linkAccountDb(id,newAccount) {
  return await apiRequest(`user-link-account?primary=${id}&secondary=${newAccount}`);
}
//unlink social accounts
export async function unlinkAccountDb(id,newAccount) {
  return await apiRequest(`user-unlink-account?primary=${id}&secondary=${newAccount}`);
}
